...
fun TextInputEditText.errorCheckingTextChanges(
    textInputLayout: TextInputLayout, @StringRes errorMessageId: Int, isValid: (String) -> Boolean
): Observable<String> {
...
fun Observable<Boolean>.mapToBoolean(): Observable<Boolean> = this.map { it }
...
object Validation {
    ...
    fun isValidAutoNumber(number: String): Boolean {
        val regex = "[0-9\\ ]{3}[A-Z\\ ]{2}[0-9\\ ]{4}[A-Z]{2}"
